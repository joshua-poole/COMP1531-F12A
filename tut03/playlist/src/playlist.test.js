...
describe('addUser tests', () => {
  test('addUser Success', () => {
    expect(addUser('hello@example.com', 'thisisaverysecurepassword')).toStrictEqual({ userId: expect.any(String) })
  })

  describe('addUser invalid email', () => {
    test('empty email', () => {
      expect(addUser('', 'thisisaverysecurepassword')).toStrictEqual({ error: "INVALID_EMAIL", message: expect.any(String)})
      expect(addUser('a', 'thisisaverysecurepassword')).toStrictEqual({ error: "INVALID_EMAIL", message: expect.any(String)})
      expect(addUser('1', 'thisisaverysecurepassword')).toStrictEqual({ error: "INVALID_EMAIL", message: expect.any(String)})
      expect(addUser('abc123@', 'thisisaverysecurepassword')).toStrictEqual({ error: "INVALID_EMAIL", message: expect.any(String)})
      expect(addUser('abc@123', 'thisisaverysecurepassword')).toStrictEqual({ error: "INVALID_EMAIL", message: expect.any(String)})
    })
  })

  test('addUser password empty', () => {
    expect(addUser('hi@hi.com', '')).toStrictEqual({ error: "INVALID_PASSWORD", message: expect.any(String)})
  })
});
